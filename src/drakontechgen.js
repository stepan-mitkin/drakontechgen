...
function createDrakonTechGenerator(options) {
  ...
  function isClass(folder) {
    return (
      (folder.type === "drakon" && folder.name === "class") || folder.isCtr
    );
  }
  ...
  function isAlgoprop(folder) {
    return (
      folder.type === "drakon" && folder.keywords && folder.keywords.algoprop
    );
  }
  ...
  function copyNode(folder, itemId, visited, targetFun) {
    ...
    if (item.type === "sinput" || isReceive(item)) {
      var stopItem = {
        ...
        type: "action",
        content:
          'self.state = "' + createStateName(item.branchName, item.id) + '"',
      };
            ...
            if (first.expressions.length !== 2) {
              ...
              if (var1.type !== "Identifier" || var2.type !== "Identifier") {
                ...
                insertActionBefore(
                  fun,
                  item.one,
                  createGetValue(item.valueVariable, item.keyVariable, second),
                );
